---
title: Agentic AI Orchestration for Task-Oriented Chatbots with Model Context Protocol
layout: mermaid
nav_order: 2
...
The present disclosure relates generally to conversational computing systems and, more particularly, to task-oriented chatbot systems that use agentic artificial intelligence, staged policy-controlled workflow execution, checkpointed fulfillment state, and standardized tool interfaces to invoke domain services.
...
In some embodiments, the orchestrator evaluates policy at multiple workflow stages, including an ingress stage for the received request, a pre-tool stage for pending tool execution, and a response stage for generated output. Policy decisions may permit execution, require confirmation, block execution, or require transformation of data. Policy decisions may further be stored as workflow-state metadata associated with the session or a related checkpoint record.

In some embodiments, the MCP service layer exposes tools through a standardized interface, validates input parameters against schemas, and invokes domain services. The orchestrator may access the tool layer through a hybrid transport abstraction that prefers native MCP protocol execution and falls back to an HTTP execution channel while preserving the same logical tool identity and correlation metadata, and while maintaining equivalent schema semantics through discovery metadata, cached schema definitions, or configured tool contracts.
...
flowchart TD
  ...
  subgraph Backend["Chat Backend"]
    AUTH["Authentication and Session<br/>Validation"]
    SSTORE[("Session Store")]
    SESS["Session Store Access<br/>(conversation history,<br/>preferences)"]
    ROUTE["Message Routing<br/>and Orchestrator Invocation"]
  ...
  subgraph Orchestrator["AI Orchestrator"]
    CTX["Session and Context<br/>Management"]
    ...
    PROMPTS["Prompt Construction<br/>(system and user prompts,<br/>examples, safety)"]
    POLICY_AI["AI Safety Policy Engine<br/>(Ingress Screening,<br/>Pre-Tool Gating,<br/>Response Sanitization)"]
    RESP["LLM Invocation and<br/>Response Post-Processing"]
  end

  subgraph MCP["MCP Service Layer"]
    VAL["Schema Validation<br/>and Parameter Checking"]
    REG["Tool Registry<br/>(names, schemas,<br/>metadata, discovery)"]
    POLICY_MCP["Optional Data Policy Engine<br/>(RBAC, API Data Rules)"]
    MASK["Masking and Redaction<br/>(sensitive fields)"]
    LOG["Tool Invocation Logging<br/>(audit, metrics)"]
  end

  subgraph Domain["Domain Services and Data"]
    SVC["Domain Microservices<br/>(Account, Transaction, Card, etc.)"]
    DB[("Primary Database")]
    AUDIT[("Audit and Observability Store")]
  end

  UI -->|user messages| AUTH
  AUTH --> SESS
  AUTH --> SSTORE
  SESS --> ROUTE
  ROUTE --> NLU_PIPE
  NLU_PIPE -->|intent and entities| ROUTE
  ROUTE -->|invoke with state| CTX
  CTX --> WF

  WF -->|tool invocations via MCP or fallback transport| VAL
  VAL --> REG
  VAL --> LOG
  LOG --> AUDIT
  ...
  MASK -->|structured and safe results| PROMPTS
...
### FIG. 2. Request Processing Flow

```mermaid
sequenceDiagram
    participant User
    participant ChatBackend as Chat Backend and NLU
    ...
    User->>ChatBackend: Message ("What is my balance?")
    ChatBackend->>ChatBackend: Authenticate and detect intent
    ChatBackend->>Orchestrator: Forward intent, entities, and context
    Orchestrator->>Orchestrator: Match workflow, restore checkpoint, extract parameters
    Orchestrator->>Orchestrator: Policy checkpoint: ingress and pre-tool
    Orchestrator->>MCP: Invoke tool request (MCP or HTTP fallback)
    MCP->>MCP: Validate schema
    MCP->>Domain: Execute business logic
    Domain-->>MCPPly: Raw domain data
    MCPPly->>MCPPly: Apply data redaction, RBAC, and masking
    ...
    Orchestrator->>Orchestrator: Policy checkpoint: redaction and leakage filtering
    Orchestrator-->>ChatBackend: Approved natural language response
    ChatBackend-->>User: Display output
```

### FIG. 3. MCP Tool Registry

```mermaid
flowchart LR
  REG3["MCP Tool Registry"] --- CFG3["Configuration Store<br/>(schemas, metadata, feature flags)"]

  subgraph REGISTRY["Registry View"]
    T1["Tool: get_accounts<br/>name; JSON schema;<br/>sensitivity tags; enabled=true"]
    T2["Tool: transfer_funds<br/>name; JSON schema;<br/>operation=write;<br/>requiresConfirmation=true"]
    T3["Tool: block_card<br/>name; JSON schema;<br/>jurisdiction rules;<br/>featureFlag=enable_block_card"]
  end
...
When policy or entity-validation results indicate that clarification or confirmation is required, the orchestrator persists a workflow checkpoint. In some embodiments, the checkpoint stores at least the workflow state, one or more pending logical tool identifiers, collected entity data, and policy decisions associated with the paused execution path. The checkpoint may be maintained in memory, in a session store, or in a durable checkpoint repository. When the requested user input is later received, the orchestrator restores the checkpoint and resumes execution without recomputing or re-planning previously completed workflow stages.
...
In some embodiments, the MCP service layer or an associated downstream policy component may additionally apply access-control, masking, redaction, filtering, tokenization, or data-minimization rules before data is returned to the orchestrator. Such processing may occur before data is included in prompts provided to a large language model.
...
In some embodiments, the orchestrator preserves the same logical tool identity across transports. Thus, the tool selected in workflow state remains the same logical tool even if the transport used to execute that tool changes. In some embodiments, parameter schema semantics and correlation metadata are likewise preserved across transport selection through shared tool definitions, discovery metadata, or cached schema contracts.
...
### Observability, Failure Handling, and Human Escalation

In one embodiment, tool invocations and policy decisions are logged together with correlation identifiers, tool names, execution status, and latency. Metrics may be collected for intent accuracy, tool success rate, response latency, and user satisfaction.

In some embodiments, retry logic, circuit-breaker controls, graceful fallback messaging, and human-agent escalation are applied when backend services fail, confidence remains low, or policy rules require human review. Such controls may be implemented within the orchestrator, within the MCP layer, or within supporting infrastructure.

### Configuration-Driven Domain Specialization

In one embodiment, the orchestrator behavior is configured using machine-readable mappings between intents, tools, prompt templates, entity rules, and validation rules. This allows the same orchestration framework to be reused across multiple domains without rewriting the core orchestration engine. New domains may be introduced by defining new intent labels, tool mappings, prompt templates, and validation configurations.
